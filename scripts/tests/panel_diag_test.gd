...
func _dump_tree(node: Node, indent: String) -> void:
	...
	if node is Control:
		...
		if r.size.x > 480.0 or r.size.y > 300.0:
			print("[BIG]%s %s%s" % [indent, name, extra])
		if r.position.x < -2.0 or r.position.y < -2.0 or r.end.x > 642.0 or r.end.y > 362.0:
...
func _check_overlap() -> void:
	var panel_rect := (_panel.get_node("Control/PanelContainer") as Control).get_global_rect()
	# HUD 中可能与面板重叠的关键元素
